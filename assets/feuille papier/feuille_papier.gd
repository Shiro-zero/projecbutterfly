...
func knock_back() -> void:
	...
	tween.parallel().tween_property(self, "position", Vector2(position.x + randf_range(-50, 50), position.y - 200), 1.5)\
		.set_trans(Tween.TRANS_SINE).set_ease(Tween.EASE_OUT)
		
	tween.parallel().tween_property(self, "rotation_degrees", rotation_degrees + randi_range(-10, 10), 1.5)\
		.set_trans(Tween.TRANS_SINE).set_ease(Tween.EASE_OUT)
